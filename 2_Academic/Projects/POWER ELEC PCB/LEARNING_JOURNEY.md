...
### Day 10 -- September 9, 2026

**Topics Covered:**
- Calculating "glue" components: Bootstrap Capacitor ($C_{boot}$), Bypass Capacitor ($C_{bypass}$), and Gate Resistors ($R_g$).
- Step-by-step Proteus simulation strategy: Testing Buck half-bridge first, then integrating the Boost side.
- Troubleshooting Boost side logic during full-circuit open-loop simulation.

**Key Takeaways:**
1. **Calculating Bootstrap Capacitor ($C_{boot}$):** Used the total gate charge ($Q_g$ = 47nC) from the IRLZ44N datasheet. Formula: $C_{boot} \ge Q_g / \Delta V_{boot}$. Assuming a 1V allowable drop, $C_{boot} \ge 47nF$. Selected a standard 1µF MLCC for a large safety margin and to handle high-frequency switching transients.
2. **Calculating Bypass Capacitor ($C_{bypass}$):** Calculated for the IR2104 VCC pin to handle peak current spikes during MOSFET turn-on. Selected 1µF ceramic capacitor placed physically close to the VCC and COM pins.
3. **Gate Resistors ($R_g$):** Added 22Ω resistors in series with HO and LO outputs to dampen ringing and control the $di/dt$ at the MOSFET gates, preventing parasitic oscillations.
4. **Modular Simulation Approach:** Instead of simulating the full 4-switch circuit at once, simulated the Buck half-bridge (Q1, Q2, U1) first. This confirmed that the bootstrap circuit charges correctly and the dead-time is present before adding complexity.
5. **Control Logic Gap in Full Simulation:** When attempting the full Buck mode simulation ($V_{in}$ = 20V, Duty = 60%), the output voltage was incorrect. Realized the Boost side (U2) was not configured in the correct static "pass-through" state (Q3 ON, Q4 OFF). The IN pin of U2 needs a specific DC level to achieve this, which was not yet defined.

**Activities:**
- [x] Calculated $C_{boot}$ (1µF), $C_{bypass}$ (1µF), and $R_g$ (22Ω) based on datasheet parameters.
- [x] Assembled and simulated the Buck half-bridge in Proteus; verified HO/LO waveforms and bootstrap charging.
- [x] Attempted full 4-switch open-loop simulation in Proteus.
- [x] Identified that the Boost side logic state needs to be explicitly defined for pass-through operation.

**Open Questions:**
- What is the exact DC voltage level required at the IN pin of IR2104 to guarantee Q3 is fully ON and Q4 is fully OFF for the Boost pass-through state? (Needs verification against IR2104 truth table).
- How to properly implement the state-machine logic in STM32 firmware to seamlessly transition between Buck, Boost, and Buck-Boost modes without output voltage discontinuity?

**Resources Used:**
- IRLZ44N Datasheet (for $Q_g$ parameter)
- IR2104(S) Datasheet (for timing and logic thresholds)
...
### Mistake #5: Starting Simulation Before Calculating "Glue" Components
**What was done:** Attempted to simulate the full power stage in Proteus without first calculating the Bootstrap Capacitor, Bypass Capacitor, and Gate Resistors.
**Result:** Simulation results were unstable or inaccurate because the gate drive circuit lacked the necessary energy storage and damping components.
